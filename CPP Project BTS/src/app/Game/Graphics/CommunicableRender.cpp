#include "CommunicableRender.h"

namespace GRAPHICS {
	CommunicableRender::CommunicableRender(sf::RenderWindow& window, std::string title, int portIn, int portOut, sf::TcpSocket& connected) : window(window), port(portIn), titleString(title) {

		// initialisation de la communication
		logDisplayer = new HELPER::LogDisplayer(CONSTANT::LOGLINESIZE + 40, CONSTANT::LOGLINECOUNT);

		communicable = new METIER::Communicable(sf::IpAddress::getLocalAddress().toString(), portIn, portOut, *logDisplayer, connected);
	}

	CommunicableRender::~CommunicableRender() {
		// delete les pointeurs
		delete communicable;
		delete ping;
		delete send;
		delete connect;
		delete log;
		delete logDisplayer;
	}

	void CommunicableRender::update(float deltaTime, sf::RenderWindow& window) {

		// update les objets
		ping->update(window);
		send->update(window);


		// update les textes
		if (communicable->isConnected()) {
			connect->setText("Connected");
			connect->setShapeColor(CONSTANT::CONNECTED_COLOR);
			connect->setTextColor(sf::Color::Black);
		}
		else {
			connect->setText("Disconnected");
			connect->setShapeColor(CONSTANT::DISCONNECTED_COLOR);
			connect->setTextColor(sf::Color::White);
		}

		connect->centerText();

		// affiche le log et recupere le log
		log->setText(logDisplayer->buildString());
	}

	void CommunicableRender::initGUI(sf::Vector2f pos, sf::Font& font, sf::Font& term, sf::TcpSocket& connected, GRAPHICS::Simulator& simulator, UI::Modal& modal) {
		// initialise les objets
		sf::Vector2f backgroundSize(window.getSize().x / 3, window.getSize().y / 2);
		float borderSize = 2.f;

		// background
		background.setSize(backgroundSize);
		background.setPosition(pos);
		background.setFillColor(sf::Color::Cyan);

		// satelliteBox
		box.setSize(backgroundSize - sf::Vector2f(20.f, 10.f));
		box.setPosition(
			HELPER::getXShapePosition(
				background.getPosition(),
				backgroundSize,
				box.getSize() + sf::Vector2f(borderSize, borderSize)
			)
		);
		
		box.setFillColor(sf::Color::Black);
		box.setOutlineThickness(borderSize);
		box.setOutlineColor(sf::Color::White);

		// initialisation des boutons

		// ping
		ping = new UI::Button(
			HELPER::getXShapePosition(
				background.getPosition(),
				backgroundSize / 2.f,
				CONSTANT::BUTTON_SIZE
			) + sf::Vector2f(0.f, box.getGlobalBounds().height / 1.3f),
			CONSTANT::BUTTON_SIZE,
			"Ping",
			font,
			CONSTANT::charSize,
			window,
			[&]() {sendCommunicable("Ping", simulator); }
		);

		// send
		send = new UI::Button(
			HELPER::getXShapePosition(
				sf::Vector2f(background.getPosition().x + backgroundSize.x / 2.f, background.getPosition().y),
				backgroundSize / 2.f,
				CONSTANT::BUTTON_SIZE
			) + sf::Vector2f(0.f, box.getGlobalBounds().height / 1.3f),
			CONSTANT::BUTTON_SIZE,
			"Send",
			font,
			CONSTANT::charSize,
			window,
			[&]() {
				modal.openModal();
				modal.setCallback([&](std::string msg) {
					sendCommunicable(msg, simulator);
					});
			}
		);

		// initialisation des textes
		connect = new UI::TextBox(
			HELPER::getXShapePosition(
				background.getPosition(),
				backgroundSize,
				CONSTANT::TEXTBOX_SIZE
			),
			CONSTANT::TEXTBOX_SIZE,
			"Disconnected",
			font,
			CONSTANT::charSize,
			window
		);

		connect->setTextColor(sf::Color::White);
		connect->setShapeColor(CONSTANT::DISCONNECTED_COLOR);

		// log
		log = new UI::TextBox(
			HELPER::getShapePosition(
				background.getPosition(),
				backgroundSize,
				CONSTANT::LOG_SIZE
			),
			CONSTANT::LOG_SIZE,
			"",
			term,
			17U,
			window
		);

		log->setTextColor(sf::Color::White);
		log->setShapeColor(sf::Color::Black);
		log->setOutlineColor(sf::Color::White);
		log->changeLocalTextPosition(sf::Vector2f(7.f, 7.f));

		// title
		title.setString(titleString + " | S:" + std::to_string(port));
		title.setFont(font);
		title.setCharacterSize(CONSTANT::charSize);
		title.setFillColor(sf::Color::White);
		title.setPosition(
			HELPER::getYShapePosition(
				box.getPosition(),
				connect->getShape().getSize(),
				title.getGlobalBounds().getSize()
			) + sf::Vector2f(10.f, 0.f)
		);
	}

	void CommunicableRender::draw(sf::RenderWindow& window) {
		// affiche les formes
		window.draw(box);
		ping->draw();
		send->draw();
		connect->draw(window);
		log->draw(window);
		window.draw(title);
	}

	void CommunicableRender::sendCommunicable(std::string msg, GRAPHICS::Simulator& simulator) {
		// envoie un message
		communicable->sendMessage(msg);

		// dessine la ligne de connexion, true pour dessiner, 2 pour 2s
		simulator.setDrawLineConnection(true, 2);
	}
}